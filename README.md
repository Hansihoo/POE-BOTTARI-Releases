...
POE BOTTARI is a community-made Windows companion for Path of Exile. It combines local PoB workflows with a Chrome extension that can translate supported build/economy sites on-device and send explicit trade-item actions to the locally installed app.
...
- Windows installer: [latest GitHub release](https://github.com/Hansihoo/POE-BOTTARI-Releases/releases/latest)
...
2. Verify the checksum if possible:
   ...
   ```powershell
   Get-FileHash .\POE-BOTTARI-Setup-community-0.1.1.exe -Algorithm SHA256
   ```
...
6. Open the extension popup. Site translation is off by default; enable only the sites you want to translate.

## Data and permissions

The extension has no advertising, analytics, or external translation service. Site translation runs locally after the user grants access to a selected site. Trade actions send only the selected item's text and current trade URL to the locally installed POE BOTTARI app.
